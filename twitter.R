...
tweets <- read.csv("~/u_tweets_2.csv", stringsAsFactors = FALSE)
...
# now let’s look at my replying habits. 
...
# Habits across 
ggplot(data = tweets, aes(x = timestamp, fill = type), binwidth=0.1) +
  geom_histogram() +
  xlab("Time") + ylab("Number of tweets") +
...
topic <- topics(lda, 1)
topics <- data.frame(date=as.IDate(tweets$created), topic)
